...
class Logger:
    def __init__(self, logdir, step, wandb_cfg=None):
        self._logdir = logdir
        disable_tb = os.environ.get("DREAMER_DISABLE_TB", "0") == "1"
        self._writer = None
        ...
        if not disable_tb:
            try:
                ...
                print(f"[Logger] TensorBoard disabled due to import error: {exc}")
        if wandb_cfg and wandb_cfg.get("enabled", False):
            try:
                ...
                print(f"[Logger] WandB enabled: {self._wandb_run.url}")
            except Exception as exc:
    ...
    def write(self, fps=False, step=False):
        ...
        if self._wandb_run is not None:
            payload = {name: value for name, value in scalars}
