...
class WorldModel(nn.Module):
    def __init__(self, obs_space, act_space, step, config):
        ...
        if self._use_stuck_head:
            self.heads["stuck"] = networks.MLP(
                feat_size,
                (),
                config.stuck_head["layers"],
                config.units,
                config.act,
                config.norm,
                dist="binary",
                outscale=config.stuck_head["outscale"],
                device=config.device,
                name="Stuck",
            )
    ...
    def _train(self, data):
        ...
        if self._use_stuck_head and "stuck" in preds and "stuck" in data:
            stuck_prob = preds["stuck"].mean
            stuck_label = data["stuck"]
            if stuck_prob.shape != stuck_label.shape:
                stuck_label = stuck_label.reshape(stuck_prob.shape)
            stuck_pred = (stuck_prob > 0.5).float()
    ...
    def video_pred(self, data):
        if "image" not in data:
            return torch.zeros((1, 1, 1, 1, 3), device=self._config.device)
        data = self.preprocess(data)
        embed = self.encoder(data)
...
class ImagBehavior(nn.Module):
    def __init__(self, config, world_model):
        ...
        self.actor = networks.MLP(
            feat_size + (1 if self._use_stuck_head else 0),
            (config.num_actions,),
            config.actor["layers"],
            config.units,
            config.act,
    ...
    def _augment_actor_feat(self, feat):
        if not self._use_stuck_head:
            return feat
        stuck_prob = self._world_model.heads["stuck"](feat).mean.detach()
        if stuck_prob.ndim < feat.ndim:
            stuck_prob = stuck_prob.unsqueeze(-1)
        return torch.cat([feat, stuck_prob], dim=-1)
    ...
    def _train(
        ...
        with tools.RequiresGrad(self.actor):
            with torch.cuda.amp.autocast(self._use_amp):
                ...
                actor_ent = self.actor(self._augment_actor_feat(imag_feat)).entropy()
    ...
    def _imagine(self, start, policy, horizon):
        ...
        def step(prev, _):
            ...
            inp = self._augment_actor_feat(feat.detach())
    ...
    def _compute_actor_loss(
        ...
        metrics = {}
        inp = self._augment_actor_feat(imag_feat.detach())
