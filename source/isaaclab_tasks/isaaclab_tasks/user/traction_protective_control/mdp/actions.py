...
class RateLimitedCarVWAction(AckermannAction):
    """Ackermann [forward velocity, steering] action with physical rate limits."""
    ...
    def __init__(self, cfg: "RateLimitedCarVWActionCfg", env):
        ...
        self._rate_limits = torch.tensor([cfg.max_v_rate, cfg.max_steer_rate], device=self.device)

    @property
    def target_actions(self) -> torch.Tensor:
        """Physical target actions after scaling and clipping, before rate limiting."""
        return self._target_actions

    @property
    def previous_executed_action(self) -> torch.Tensor:
        """Physical actions applied during the latest simulation step."""
        return self._previous_executed_action

    @property
    def previous_target_action(self) -> torch.Tensor:
        """Previous unclamped physical target, useful for smoothness rewards."""
        return self._previous_target_action
    ...
    def process_actions(self, actions: torch.Tensor):
        ...
        self._previous_target_action[:] = self._target_actions
        self._target_actions[:] = target

        step_dt = float(self._env.step_dt)
        max_delta = self._rate_limits * step_dt
        action_delta = torch.clamp(target - self._previous_executed_action, min=-max_delta, max=max_delta)
    ...
    def reset(self, env_ids: Sequence[int] | None = None) -> None:
        ...
        if self.cfg.reset_to_zero:
            ...
            self._previous_target_action[env_ids] = 0.0
...
class RateLimitedCarVWActionCfg(AckermannActionCfg):
    ...
    """Maximum forward velocity command rate in m/s^2."""

    max_steer_rate: float = 2.0
    """Maximum steering command rate in rad/s."""

    reset_to_zero: bool = True
    """Whether to reset executed action state to zero at episode reset."""
