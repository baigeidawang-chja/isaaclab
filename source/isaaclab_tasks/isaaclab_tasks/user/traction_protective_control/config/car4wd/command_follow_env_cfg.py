...
from ...mdp import PlannerCommandCfg
...
@configclass
class CommandFollowSceneCfg(InteractiveSceneCfg):
    """Flat high-traction scene for Car4WD command following."""
...
@configclass
class CommandsCfg:
    """Upper-level planner speed and heading commands."""
...
@configclass
class ActionsCfg:
    """Policy controls executed forward velocity and steering angle."""
...
class EventCfg:
    ...
    reset_runtime_buffers = EventTerm(func=events.reset_runtime_buffers, mode="reset", params={})


@configclass
class RewardsCfg:
    speed_tracking = RewTerm(
        func=rewards.speed_tracking,
        weight=2.0,
    ...
    yaw_rate = RewTerm(func=rewards.yaw_rate_penalty, weight=0.1, params={"scale": 0.05})
    lateral_velocity = RewTerm(func=rewards.lateral_velocity_penalty, weight=0.1, params={"scale": 0.05})
...
class TerminationsCfg:
    ...
    out_of_bounds = DoneTerm(
        ...
        time_out=False,
        params={"x_min": -20.0, "x_max": 20.0, "y_min": -10.0, "y_max": 10.0},
    )


@configclass
class CommandFollowEnvCfg(ManagerBasedRLEnvCfg):
    """First-stage flat speed and heading command-following task."""

    scene: CommandFollowSceneCfg = CommandFollowSceneCfg(num_envs=1, env_spacing=3.0)
...
@configclass
class CommandFollowEnvCfg_PLAY(CommandFollowEnvCfg):
    def __post_init__(self):
        super().__post_init__()
