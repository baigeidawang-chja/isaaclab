...
def _warn_contact_fallback_once(env, reason: str):
    if not getattr(env, "_medium_state_contact_fallback_warned", False):
        print(
            "[MEDIUM STATE] Wheel contact unavailable; "
            f"fallback eta_wheel = 1 - wheel_submersion ({reason})."
        )
        env._medium_state_contact_fallback_warned = True


def _compute_wheel_contact_factor(
    env,
    num_envs: int,
    device: torch.device,
    contact_force_ref: float,
    contact_force_threshold: float,
    use_binary_wheel_contact: bool,
) -> torch.Tensor:
    """Return mean wheel contact factor in [0, 1], or ones if contact data is unavailable."""
    sensor = env.scene.sensors.get("robot_contact_sensor", None)
    if sensor is None:
        _warn_contact_fallback_once(env, "missing robot_contact_sensor")
        return torch.ones(num_envs, device=device, dtype=torch.float32)
    try:
        forces = sensor.data.net_forces_w
    except (AttributeError, RuntimeError) as exc:
        _warn_contact_fallback_once(env, f"sensor data not ready: {type(exc).__name__}")
        return torch.ones(num_envs, device=device, dtype=torch.float32)
    if forces is None or forces.numel() == 0:
        _warn_contact_fallback_once(env, "empty net_forces_w")
        return torch.ones(num_envs, device=device, dtype=torch.float32)

    force_norm = torch.norm(forces, dim=-1).to(device=device, dtype=torch.float32)
    num_bodies = force_norm.shape[1]
    body_names = list(getattr(sensor, "body_names", []))
    wheel_ids = [
        idx
        for idx, name in enumerate(body_names)
        if any(token in name.lower() for token in ("wheel", "tire", "contact"))
    ]
    if len(wheel_ids) >= 4:
        selected = force_norm[:, torch.as_tensor(wheel_ids[:4], device=device, dtype=torch.long)]
    else:
        take = min(4, num_bodies)
        if take <= 0:
            _warn_contact_fallback_once(env, "no contact bodies")
            return torch.ones(num_envs, device=device, dtype=torch.float32)
        selected = torch.topk(force_norm, k=take, dim=1).values

    if use_binary_wheel_contact:
        contact = (selected > float(contact_force_threshold)).float()
    else:
        contact = torch.clamp(selected / max(float(contact_force_ref), 1e-6), 0.0, 1.0)
...
def compute_medium_state(
    ...
    if medium_mode == "linear":
        ...
        wheel_contact_factor = _compute_wheel_contact_factor(
            env,
            num_envs=root_pos_w.shape[0],
            device=device,
        ...
        eta_wheel = wheel_contact_factor * (1.0 - wheel_submersion)
        ...
        return {
            ...
            "wheel_contact_factor": wheel_contact_factor,
